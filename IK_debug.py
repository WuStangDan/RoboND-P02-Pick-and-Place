...
def test_code(test_case):
    ...
    eulers = tf.transformations.euler_from_quaternion([orientation.x,
                             orientation.y, orientation.z, orientation.w])
    Rrpy = rot_z(eulers[2]) * rot_y(eulers[1]) * rot_x(eulers[0]) * R_cor

    your_wc = [1,1,1]
    your_wc[0] = position.x - s[d7] * Rrpy[0,2]
    your_wc[1] = position.y - s[d7] * Rrpy[1,2]
    your_wc[2] = position.z - s[d7] * Rrpy[2,2]
    #print("WC", your_wc)
